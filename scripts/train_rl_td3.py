...
def find_latest_checkpoint(ckpt_dir):
    ...
    # CHANGED pattern from 'ddpg' to 'td3'
    pattern = re.compile(r'td3_franka_(\d+)_steps\.zip')
    steps = []
    ...
    # CHANGED filenames from 'ddpg' to 'td3'
    model_path = os.path.join(ckpt_dir, f'td3_franka_{latest_step}_steps.zip')
    vec_path = os.path.join(ckpt_dir, f'td3_franka_vecnormalize_{latest_step}_steps.pkl')
...
if __name__ == "__main__":
    rospy.init_node('TD3_trainer', anonymous=True) # CHANGED node name
    ...
    # --- Configuration ---
    GAMMA = 0.99
    MAX_CPP_EPISODES =2000  # Set the total number of episodes for the C++ node to run
    ...
    if args.new:
        ...
        if latest_run:
            ...
            # CHANGED final model name
            load_model_path = os.path.join(MODELS_DIR, run_id, "td3_franka_model.zip")
    ...
    # CHANGED final save paths
    model_final_save_path = os.path.join(model_save_dir, "td3_franka_model.zip")
    ...
    if is_model_loaded:
        ...
        rospy.loginfo(f"Loading TD3 model from: {load_model_path}") # CHANGED log message
        model = TD3.load(load_model_path, env=norm_vec_env) # CHANGED from DDPG.load
        ...
        rospy.loginfo("Creating a new TD3 model.") # CHANGED log message
        n_actions = norm_vec_env.action_space.shape[-1]
        action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=0.1 * np.ones(n_actions))
        # CHANGED to create a TD3 model
        model = TD3("MlpPolicy", norm_vec_env, action_noise=action_noise, verbose=1, 
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="td3_franka", # CHANGED name prefix
    ...
    # --- Train The Model ---
    rospy.loginfo(f"Starting TD3 training. Run ID: {run_id}") # CHANGED log message
