...
std::vector<std::vector<double>> readCSV(const std::string& filename) 
{
    ...
    while (std::getline(file, line)) {
        ...
        while (std::getline(ss, value, ',')) {
            // std::cout << "ori value" << value;
            try {
                double number = std::stod(value);
                // std::cout << "parsed as double" << number << std::endl;
...
// Helper function to capture camera_value data synchronously
sensor_msgs::JointState::ConstPtr getCameraValue(const std::string& topic_name) {
    ROS_INFO("Waiting for camera_value message...");
    auto msg = ros::topic::waitForMessage<sensor_msgs::JointState>(topic_name, ros::Duration(5.0));
    if (!msg) {
        ROS_WARN("Timeout waiting for camera_value message.");
    }
    return msg;
}

void writeMinDistancesToCSV(moveit::planning_interface::MoveGroupInterface& move_group_interface,
                            ...
                            bool append = true) {
    // Open the file in append mode or write mode based on the flag
    std::ofstream csv_file;
    bool file_already_exists = fileExists(file_path);
    csv_file.open(file_path, std::ios::app); // Append mode
    ...
    // Retrieve camera_value data synchronously
    auto camera_msg = getCameraValue(camera_topic_name);
    if (!camera_msg) {
        std::cerr << "Error: No camera_value data received." << std::endl;
        return;
    }
    ...
    if (!file_already_exists) {
        for (size_t i = 0; i < 25; ++i) {
            csv_file << "X_" << i << ",Y_" << i << ",D_" << i;
            if (i < 24) { // Add a comma only if not the last column
                csv_file << ",";
            }
        }
        ...
        csv_file << ",Waypoints,Action,JointIndex,MinDistance,Label\n";
    }

    // Write camera data (positions, velocities, efforts)
    for (size_t i = 0; i < camera_msg->position.size(); ++i) {
        double position = camera_msg->position[i];
        double velocity = (i < camera_msg->velocity.size()) ? camera_msg->velocity[i] : 0.0;
        double effort = (i < camera_msg->effort.size()) ? camera_msg->effort[i] : 0.0;

        csv_file << position << "," << velocity << "," << effort << ",";
    }
    ...
    for (size_t i = 0; i < min_distances.size(); ++i) {
        if (!std::isnan(min_distances[i]) && min_distances[i] < min_distance) {
            min_distance = min_distances[i];
            min_index = i;
        }
    }

    // Determine the label for the minimum distance
    std::string label = (min_distance < threshold) ? "1" : "0";

    // Write the minimum value and its index to the file
    csv_file << filename << "," << action_data << "," << min_index << "," << min_distance << "," << label << "\n";
...
std::vector<double> calculateMinDistances(moveit::planning_interface::MoveGroupInterface& move_group_interface, const std::string& filename, const std::vector<Eigen::Vector3d>& human_joint_positions,const std::string& title, std::vector<Eigen::Vector3d>& plan_trajectory_point, int action_data){
    ...
    double threshold = 0.2;  // Define the threshold
    std::string csv_path = expFolderPath + "/label.csv";
    std::string camera_topic_name = "camera_value";
    writeMinDistancesToCSV(move_group_interface, filename, min_distances, csv_path, threshold, action_data, camera_topic_name);
...
void hoverPose(moveit::planning_interface::MoveGroupInterface& move_group_interface, float x, float y, int data)
{ 
  ...
  target_pose_hover.position.z = 1.3;
...
//   std::cout << "saved_filename =" << saved_filename << std::endl;
  ...
  if (!saved_filename.empty()) {
            auto min_distances = calculateMinDistances(move_group_interface, saved_filename, human_joint_positions, title, plan_trajectory_point, data);
            // for (size_t i = 0; i < min_distances.size(); ++i) {
...
void pickPose(moveit::planning_interface::MoveGroupInterface& move_group_interface, std::string direction, float x, float y, int data)
{
  moveit::planning_interface::MoveGroupInterface::Plan cartesianPlan;
  move_group_interface.setStartStateToCurrentState();
  ...
  if (direction == "down"){
    ...
    target_pose_pick.position.z = 1.1; //target position
  }
  else if (direction == "up"){
    target_pose_pick.position.z = 1.3;
  }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (direction == "down"){
    ...
    if (!saved_filename.empty()) {
            auto min_distances = calculateMinDistances(move_group_interface, saved_filename, human_joint_positions,sub_title,plan_trajectory_point, data);
            // for (size_t i = 0; i < min_distances.size(); ++i) {
            //     // std::cout << "Minimum distance between robot and human keypoints " << i << ": " << min_distances[i] << std::endl;
            // }
        }
...
void hoverPlacePose(moveit::planning_interface::MoveGroupInterface& move_group_interface, float x, float y, int data)
{ 
...
void PlacePose(moveit::planning_interface::MoveGroupInterface& move_group_interface, std::string direction,float x, float y, int data)
{ 
  moveit::planning_interface::MoveGroupInterface::Plan cartesianPlan;
  move_group_interface.setStartStateToCurrentState();
  ...
  if (direction == "down"){
    target_pose_place.position.z = 1.09; 
   
  }

  else if (direction == "up"){
    target_pose_place.position.z = 1.3;
  }
...
std::uniform_real_distribution<> dis(min, max); // Distribution range
double value = dis(gen);
return std::round(value * 100.0) / 100.0; // Round to 2 decimal places
}
...
group_manipulator.setMaxVelocityScalingFactor(0.2);
group_manipulator.setMaxAccelerationScalingFactor(0.2);

// Add collision objects to the planning scene
addCollisionObjects(planning_scene_interface, 0.3, -0.3);

ros::WallDuration(1.0).sleep();

// Main loop for 10 iterations
for (int iteration = 0; iteration < 550; ++iteration) {
    ROS_WARN("Starting iteration: %d", iteration + 1);

    // Call createNextExpFolder for each iteration
    std::string basePath = "/home/franka/RELAX_franka/src/pick_and_place/experiment/";
    createNextExpFolder(basePath);

    // Pause to ensure folder creation and setup
    ...
    // Inside your main function or where hoverPose is called:
    double random_x = getRandomValue(0.35, 0.55); // Generate random value
    double random_y1 = getRandomValue(-0.05, 0.05);
    double random_y2 = getRandomValue(0.15, 0.25);
    
    state.data = 1;
    pose_state_pub.publish(state);
    ros::Duration(2).sleep();
    hoverPose(group_manipulator, random_x, random_y1, state.data);

    state.data = 2;
    pose_state_pub.publish(state);
    ros::Duration(2).sleep();
    pickPose(group_manipulator, "down", random_x, random_y1, state.data);
    pick(group_arm);
    ...
    pose_state_pub.publish(state);
    ros::Duration(2).sleep();
    pickPose(group_manipulator, "up", random_x, random_y1, state.data);

    state.data = 4;
    pose_state_pub.publish(state);
    ros::Duration(2).sleep();
    hoverPlacePose(group_manipulator, random_x, 0-random_y2, state.data);

    state.data = 5;
    pose_state_pub.publish(state);
    ros::Duration(2).sleep();
    PlacePose(group_manipulator, "down", random_x, 0-random_y2, state.data);

    state.data = 6;
    pose_state_pub.publish(state);
    ros::Duration(2).sleep();
    PlacePose(group_manipulator, "up", random_x, 0-random_y2, state.data);
    ...
    ROS_WARN("Round %d end", iteration + 1);
    }
