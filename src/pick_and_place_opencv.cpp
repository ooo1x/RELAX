...
#include <limits>
#include <random>
#include <pick_and_place/matplotlibcpp.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
#include <thread>
...
// std::string csv_file_path = "/home/yuliang/franka_openpose/src/pick_trajectory/";
...
// std::vector<Eigen::Vector3d> calculateEEPosition(const std::string& csv_filename) 
...
std::vector<Eigen::Vector3d> calculateEEPosition(const std::string& csv_filename, std::vector<Eigen::Vector3d>& plan_trajectory_point) 
{
    ...
    std::cout << "Filename: " << csv_filename << std::endl;
